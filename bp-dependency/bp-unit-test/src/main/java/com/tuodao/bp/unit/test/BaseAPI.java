...
public class BaseAPI {
	...
	// 对内
    protected String url = "http://127.0.0.1:8088/api/router/";
	// 对外
...
//            headers.set("reqlength", String.valueOf(requestBody.length()));
            headers.set("accessId", accessId);
            headers.set("accessKey", accessKey);
            headers.set("sign","NO");
            headers.set("requestType","PC");
    ...
    public <T> String doServiceCalc(String action, ContentType type, T content){
        String requestUrl = url + "/" +action + "." + type.name().toLowerCase();
        ...
        try{
            ...
            headers.set("sign", SignEncryptUtils.getDefaultSign(accessId + timestamp + accessKey));
            headers.set("requestType","PC");
        }catch(Exception e){
            throw new RuntimeException(e);
        }
