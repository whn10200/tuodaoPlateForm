...
public class CoreHandlerMethodArgumentResolver implements HandlerMethodArgumentResolver {
	...
	public Object resolveArgument(MethodParameter parameter, ModelAndViewContainer mavContainer,
		...
		String requestIp = Joiner.on("").skipNulls().join("请求获取Ip: ", ip);

		userOperateLog.setRequesetIp(requestIp);

		if(CoreHandlerInterceptor.getBankCurrent()){
			logger.info("银行回调请求...不做任何参数检验处理...");
		 ...
		 * 设置用户编号
		 */
		String accessId = webRequest.getHeader("accessId");
		/* 解决无法注入的问题,不得已而为*/
		// BeanFactory factory = WebApplicationContextUtils.getRequiredWebApplicationContext(request.getServletContext());
		// AccessCache accessCache = (AccessCache) factory.getBean("accessCache");
		AccessInfo accessInfo = accessCache.getAccessInfo(accessId);
		if (null == accessInfo || StringUtils.isEmpty(accessInfo.getUserId())) {
			accessInfo = accessCache.getAccessInfoApp(accessId);
			if (null == accessInfo || StringUtils.isEmpty(accessInfo.getUserId())) {
				return input;
			}else {
				return getObject(input, accessInfo);
			}
		}
		return getObject(input, accessInfo);
	}

	private Object getObject(Object input, AccessInfo accessInfo) {
		BasePojo basePojo = (BasePojo)input;
		basePojo.setUserId(accessInfo.getUserId());
		basePojo.setMobile(accessInfo.getMobile());
		BasePojo inputCurrent = CoreHandlerInterceptor.getInputCurrent();
		basePojo.setRequestType(inputCurrent.getRequestType());
		basePojo.setVersion(inputCurrent.getVersion());

		// 记录操作日志
		SystemOperateLog userOperateLog = userOperateLogComponent.currentUserOperateLog();
		userOperateLog.setUserId(accessInfo.getUserId());
		userOperateLog.setMobile(accessInfo.getMobile());
		userOperateLog.setServiceType(inputCurrent.getRequestType());

		String ip = Joiner.on("").skipNulls().join(userOperateLog.getRequesetIp(), " / 接口请求输入Ip: ", basePojo.getIp());
		userOperateLog.setRequesetIp(ip);
